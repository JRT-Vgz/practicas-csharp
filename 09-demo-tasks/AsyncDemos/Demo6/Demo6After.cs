...
{
    public partial class Demo6After : Form
    {
        public Demo6After()
        {
            InitializeComponent();
        ...
        private async void btnSearch_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                //var amiibos = GetAll();
                var terms = txtTerm.Text.Split(new[] { ',' }, StringSplitOptions.RemoveEmptyEntries);

                var tasks = new List<Task<List<Amiibo>>>();
                ...
                //var amiibos = (await Task.WhenAll(tasks)).SelectMany(x=>x);
                var result = await Task.WhenAny(tasks);
                var amiibos = await result;
